 ...
 */
public class PushBotHardware3 extends LinearOpMode

{
    ...
    public void runOpMode() throws InterruptedException {
        ...
        try
        {
            v_servo_flag = hardwareMap.servo.get ("flag_servo");
            //      v_servo_left_hand.setPosition (l_hand_position);
        }
        catch (Exception p_exeception)
        {
            m_warning_message ("flag_servo");
            DbgLog.msg (p_exeception.getLocalizedMessage ());
    ...
    double a_flag_position ()
    {
        double l_return = 0.0;

        if (v_servo_flag != null)
        {
            l_return = v_servo_flag.getPosition ();
        }
    ...
    void m_flag_position(double p_position)
    {
        double l_position = Range.clip
                ( p_position
                        , Servo.MIN_POSITION
        ...
        if (v_servo_flag != null)
        {
            v_servo_flag.setPosition (l_position);
        }
    }
    ...
    void engage_jammer ()
    {
        if (v_servo_jammer != null)
        {
            v_servo_jammer.setPosition (Servo.MAX_POSITION);
        }
        isEngaged = true;
    }
    void disengage_jammer ()
    {
        if (v_servo_jammer != null)
        {
            v_servo_jammer.setPosition(Servo.MIN_POSITION);
        }
        isEngaged =false;
    ...
    private Servo v_servo_jammer;
    private Servo v_servo_flag;
